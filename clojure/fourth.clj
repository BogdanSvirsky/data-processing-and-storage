...
(defn notify-msg
  ...
  (let [bill (:bill state)
    ...
    (if-not (contains? bill ware)
      ...
      (let [need (- (bill ware) (buffer ware))
            take (min need amount)]
        (if (pos? take)
          (try
            (swap! storage-atom #(- % take))

            (let [new-buffer (update buffer ware + take)
                  new-state (assoc state :buffer new-buffer)]

              (if (every?
                    (fn [[w req]] (>= (new-buffer w) req))
                    bill)
                    ...
                    *agent*
                    (fn [s]
                      ...
                      (let [cleared-buffer
                        ...
                        (send ((:target-storage s) :worker)
                              supply-msg
                              (:amount s))

                        (assoc s :buffer cleared-buffer))))
